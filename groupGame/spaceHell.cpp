...
void SpaceHell::initAssets()
{
	...
	//background animation source: https://wallpaperaccess.com/pixel-space
...
void SpaceHell::initPlayer()
{
	...
	*/
	...
	player->setRotate(0.f);

	//set the spawn animation
	player->setSpawnAnimation("spawnAnimation", assets["playerSpawn"], 4, 0, 64, 64);
}
...
void SpaceHell::resetGame(bool returnToMainMenu)
{
	...
	if (bossBGM.getStatus() == sf::Music::Playing)
		bossBGM.stop();
	if (sounds["emergency"]->getStatus() == sf::Sound::Playing)
...
void SpaceHell::update()
{
	...
	updateWorldCollision(player, 0.f);
...
void SpaceHell::updatePlayer()
{
	...
	playerPos = sf::Vector2f(player->getPos().x, player->getPos().y);
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space) && player->canFire())
	{
		...
		if (playerFirePattern == 0)
		{
			...
			sf::Texture* newTexture = assets["playerBullet"];
			int bulletType = 0;
			float bulletSpeed = 8.f * scale;
			playerBullets.push_back(new Bullet(*newTexture, bulletType, bulletHitbox, playerPos, mouseAngle, playerAimDir, bulletSpeed, scale));
		}
		...
		else if (playerFirePattern == 3)
		{
			...
			sf::Texture* bulletTexture = assets["bigPlayerBullet"];
...
void SpaceHell::updateBullets()
{
	...
	*/
	...
	if (playerFireCluster)
	{
		playerFireCluster = false;
		sf::FloatRect bulletHitbox(1, 1, 6, 6);
		burstClusterShot("player", playerBulletPos, assets["playerBullet"], bulletHitbox, 10, 10.f * scale);
	}
	...
	*/
	...
	if (enemyFireCluster)
	{
		enemyFireCluster = false;
		sf::FloatRect bulletHitbox(1, 1, 6, 6);
		burstClusterShot("enemy", enemyBulletPos, assets["enemyBullet"], bulletHitbox, 10, 3.f * scale);
	}
}
...
void SpaceHell::updateEnemyCollision()
{
	...
	for (auto *enemy : enemies)
	{
		...
		for (auto *bullet : playerBullets)
		{
			...
			if (enemyCollision)
			{
				...
				if (bullet->getType() == 12)
				{
					...
					float clusterDamage = 0.1f + (0.1f * 0.2f * (totalPlayerDamageUp * 5));
		...
		if (enemyDestroyed || bossDestroyed)
		{
			...
			explosions.push_back(new Explosion(assets["destroyedSheet"], enemy->getPos(), scale));
			...
			//delete the enemy
			delete enemies[enemyVectPos];
			enemies.erase(enemies.begin() + enemyVectPos);
			--enemyVectPos;
		}
		bulletVectPos = 0;
		enemyVectPos++;
...
void SpaceHell::updatePlayerCollision()
{
	...
	for (auto *enemyBullet : enemyBullets)
	{
		...
		if (!invincible && !playerInvincible)
		{
			...
			bool playerCollision = enemyBullet->getGlobalHitbox().intersects(player->getGlobalHitbox());
			...
			if (playerCollision)
			{
				...
				delete enemyBullet;
				enemyBullets.erase(enemyBullets.begin() + bulletVectPos);
			...
			for (auto *enemy : enemies)
			{
				...
				playerCollision = enemy->getGlobalHitbox().intersects(player->getGlobalHitbox());
...
void SpaceHell::updateUpgradeCollision()
{
	...
	while (itr != upgrades.end())
	{
		...
		bool upgradeCollision = (*itr)->getGlobalHitbox().intersects(player->getGlobalHitbox());
...
void SpaceHell::updateEnemyAnimation(Enemy* enemy)
{
	...
	else if (enemy->getCurrentAnimation() == "spawn")
	{
		...
		{
		case 0:
			enemy->setIdleAnimation("idleAnimation", assets["enemyOneIdle"], 11, 0, 32, 32);
			break;
		case 10:
			enemy->setIdleAnimation("idleAnimation", assets["bounceEnemyOneIdle"], 11, 0, 32, 32);
			break;
		case 1:
			enemy->setIdleAnimation("idleAnimation", assets["enemyTwoIdle"], 11, 0, 32, 32);
			break;
		case 11:
			enemy->setIdleAnimation("idleAnimation", assets["circleEnemyTwoIdle"], 11, 0, 32, 32);
			break;
		case 2:
			enemy->setIdleAnimation("idleAnimation", assets["enemyThreeIdle"], 11, 0, 32, 32);
			break;
		case 12:
			enemy->setIdleAnimation("idleAnimation", assets["burstEnemyThreeIdle"], 11, 0, 32, 32);
			break;
		case 3:
			enemy->setIdleAnimation("idleAnimation", assets["enemyFourIdle"], 11, 0, 32, 32);
			break;
		case 100:
			enemy->setIdleAnimation("idleAnimation", assets["bossEnemyIdle"], 11, 0, 64, 64);
			break;
		}
	}
...
void SpaceHell::updatePlayerAnimation()
{
	...
	{
		player->setIdleAnimation("idleAnimation", assets["playerIdle"], 10, 0, 64, 64);
	}
	...
	{
		player->setDamagedAnimation("damagedAnimation", assets["playerDamaged"], 10, 0, 64, 64);
	}
...
void SpaceHell::renderGame()
{
	...
	if (player->getHp() <= 0.f && !playerDestroyed)
	{
...
void SpaceHell::gameDebugMode()
{
	...
	if (canSelect)
	{
		...
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F1))
		{
			...
			upgrades.push_back(new Upgrade(assets["damageUpIdle"], upgradeSpawnPos, 30.f, 1, deltaTime, scale));
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F2))
		{
			//spawn hp up
			std::cout << "hp upgrade dropped\n";
			upgrades.push_back(new Upgrade(assets["hpUpIdle"], upgradeSpawnPos, 30.f, 2, deltaTime, scale));
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F3))
		{
			//spawn fire rate up
			std::cout << "fire rate upgrade dropped\n";
			upgrades.push_back(new Upgrade(assets["fireRateUpIdle"], upgradeSpawnPos, 30.f, 3, deltaTime, scale));
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F4))
		{
			//spawn double bullets
			std::cout << "double bullets upgrade dropped\n";
			upgrades.push_back(new Upgrade(assets["doubleBulletUpIdle"], upgradeSpawnPos, 30.f, 4, deltaTime, scale));
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F5))
		{
			//spawn spread bullets
			std::cout << "spread bullets upgrade dropped\n";
			upgrades.push_back(new Upgrade(assets["spreadBulletUpIdle"], upgradeSpawnPos, 30.f, 5, deltaTime, scale));
		}
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::F6))
		{
			//spawn cluster bullets
			std::cout << "cluster bullet upgrade dropped\n";
			upgrades.push_back(new Upgrade(assets["burstBulletUpIdle"], upgradeSpawnPos, 30.f, 6, deltaTime, scale));
		}
...
void SpaceHell::spawnEnemyOne(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	enemies.push_back(new Enemy(*assets["enemyOne"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	//set the enemies animation to spawn when spawned
	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["enemyOneSpawn"];
...
void SpaceHell::spawnBounceEnemyOne(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	//play enemy spawn sound
	sounds["enemySpawn"]->play();
	enemies.push_back(new Enemy(*assets["bounceEnemyOne"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["bounceEnemyOneSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnEnemyTwo(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	//play enemy spawn sound
	sounds["enemySpawn"]->play();
	enemies.push_back(new Enemy(*assets["enemyTwo"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["enemyTwoSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnCircleEnemyTwo(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	float movementSpeed = 0.8f * scale;
	int type = 11;
	enemies.push_back(new Enemy(*assets["circleEnemyTwo"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["circleEnemyTwoSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnEnemyThree(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	int type = 2;
	enemies.push_back(new Enemy(*assets["enemyThree"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));


	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["enemyThreeSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnBurstEnemyThree(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	float movementSpeed = 1.0f * scale;
	int type = 12;
	enemies.push_back(new Enemy(*assets["burstEnemyThree"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["burstEnemyThreeSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnEnemyFour(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	int type = 3;
	enemies.push_back(new Enemy(*assets["enemyFour"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, scale));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["enemyFourSpawn"];
	enemy->setSpawnAnimation("spawnAnimation", spawnTexture, 4, 0, 32, 32);
}
...
void SpaceHell::spawnBoss(sf::Vector2f initialPosition, unsigned posInWave)
{
	...
	enemies.push_back(new Enemy(*assets["bossEnemy"], type, hp, damage, fireRate, movementSpeed, newHitbox, initialPosition, posInWave, size));

	Enemy* enemy = enemies.back();
	sf::Texture* spawnTexture = assets["bossEnemySpawn"];
...
void SpaceHell::enemyOneFirePattern()
{
	...
	enemyBullets.push_back(new Bullet(*assets["enemyBullet"], bulletType, bulletHitbox, enemyPos, enemyAngle, enemyAimDir, bulletSpeed, scale));
}
...
{
	sf::FloatRect bulletHitbox(1, 1, 6, 6);
	sf::Texture* bulletTexture = assets["bouncingEnemyBullet"];
	int bulletType = 13;
	float bulletSpeed = 3.f * scale;
...
{
	sf::FloatRect bulletHitbox(1, 1, 6, 6);
	sf::Texture* bulletTexture = assets["enemyBullet"];
	int bulletType = 10;
...
{
	sf::Texture* bulletTexture = assets["bigEnemyBullet"];
	sf::FloatRect bulletHitbox(1, 1, 14, 14);
	int bulletType = 12;
...
void SpaceHell::fireSpread(const std::string character, sf::Texture* bulletTexture, sf::FloatRect bulletHitbox, int bulletType, float bulletSpeed, float spreadDegree)
{
	...
	if (character == "player")
	{
		...
		playerBullets.push_back(new Bullet(*bulletTexture, bulletType, bulletHitbox, playerPos, mouseAngle, newPlayerAimDir, bulletSpeed, scale));
	...
	else
	{
		...
		enemyBullets.push_back(new Bullet(*bulletTexture, bulletType, bulletHitbox, enemyPos, enemyAngle, newEnemyAimDir, bulletSpeed, scale));
...
void SpaceHell::dropPowerUp(Enemy* enemy)
{
	...
	if (randChance <= dropChance)
	{
		...
		{
			newTexture = assets["damageUpIdle"];
			dropType = 1;
		}
		//spawn hp up
		else if (dropType <= hpUpChance)
		{
			newTexture = assets["hpUpIdle"];
			dropType = 2;
		}
		//spawn fire rate up
		else if (dropType <= fireRateUpChance)
		{
			newTexture = assets["fireRateUpIdle"];
			dropType = 3;
		}
		//spawn double bullets
		else if (dropType <= doubleBulletUpChance)
		{
			newTexture = assets["doubleBulletUpIdle"];
			dropType = 4;
		}
		//spawn spread bullets
		else if (dropType <= spreadBulletUpChance)
		{
			newTexture = assets["spreadBulletUpIdle"];
			dropType = 5;
		}
		//spawn burst bullets
		else if (dropType <= burstBulletUpChance)
		{
			newTexture = assets["burstBulletUpIdle"];
			dropType = 6;
		}
